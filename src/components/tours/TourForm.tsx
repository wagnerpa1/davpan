...
interface GuidesSectionProps {
  addGuide: (id: string, name: string) => void;
  guides?: { id: string; full_name: string }[];
  removeGuide: (id: string) => void;
  selectedGuides: SelectedGuide[];
}
...
function handleStartDateChange(event: React.ChangeEvent<HTMLInputElement>) {
  ...
  if (endDateInput instanceof HTMLInputElement) {
    endDateInput.min = event.currentTarget.value;

    if (!endDateInput.value || endDateInput.value < event.currentTarget.value) {
      endDateInput.value = event.currentTarget.value;
    }
  }
}
        ...
        <div className="flex flex-wrap gap-2 min-h-10 p-2 rounded-xl bg-slate-50 border border-dashed border-slate-200">
          {selectedGuides.map((guide) => (
            ...
            >
              ...
              <button
                type="button"
                aria-label={`Leiter ${guide.name} entfernen`}
...
function MaterialsSection({
  ...
  return (
    <div className="space-y-4 rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
      ...
      <p className="text-xs text-slate-500 mb-2">
        Wähle Materialien aus dem Bestand, die Teilnehmer für diese Tour direkt
        mitbuchen können.
      </p>
          ...
          availableMaterials.map((material) => {
            const isSelected =
              initialData?.tour_materials?.some(
                (tourMaterial) => tourMaterial.material_id === material.id,
              ) || false;
            return (
              <label
                key={material.id}
  ...
  return (
    <div className="space-y-4 rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
      ...
      <p className="text-sm text-slate-500 mb-2">
        ...
        benötigst. Die Reservierung erfolgt automatisch für den Tour-Zeitraum,
        sofern verfügbar.
      </p>
        ...
        {availableResources.map((resource) => {
          ...
          return (
            <label
              key={resource.id}
              className={cn(
                "flex items-center gap-3 rounded-xl border p-3 cursor-pointer transition-colors hover:bg-slate-50",
                isSelected
...
function TimingSection({ initialData, onStartDateChange }: TimingSectionProps) {
  return (
    <div className="space-y-4 rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
      <h3 className="text-lg font-bold flex items-center gap-2 text-slate-900 border-b border-slate-50 pb-2">
...
function TourFormActions({
  initialData,
  isLoading,
  onCancel,
}: TourFormActionsProps) {
  return (
...
export function TourForm({
  ...
  const [selectedGuides, setSelectedGuides] = useState<SelectedGuide[]>(() => {
    if (initialData?.tour_guides) {
      return initialData.tour_guides.map((tourGuide) => ({
        id: tourGuide.user_id,
        name: tourGuide.profiles?.full_name || "Unbekannt",
      }));
    }
  ...
  const [selectedResources, setSelectedResources] = useState<string[]>(() => {
    if (initialData?.resource_bookings) {
      return initialData.resource_bookings.map(
        (resourceBooking) => resourceBooking.resource_id,
      );
    }
    return [];
  });
  ...
  const selectedResourceIds = useMemo(
    () => new Set(selectedResources),
    [selectedResources],
  );

  const addGuide = (id: string, name: string) => {
    if (!id) return;
    if (selectedGuides.some((guide) => guide.id === id)) return;
    setSelectedGuides([...selectedGuides, { id, name }]);
  };

  const removeGuide = (id: string) => {
    setSelectedGuides(selectedGuides.filter((guide) => guide.id !== id));
  };

  return (
    <form action={onSubmit} className="space-y-8">
      {selectedGuides.map((guide) => (
        <input key={guide.id} type="hidden" name="guide_ids" value={guide.id} />
      ))}
      {initialData?.tour_materials?.map((material) => (
        <input
          key={`init-m-${material.material_id}`}
          type="hidden"
      ...
      />
      <ParametersSection initialData={initialData} />
      <OrganizationSection initialData={initialData} />
      <TourFormActions
        initialData={initialData}
        isLoading={isLoading}
