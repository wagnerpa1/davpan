...
function useTourFilters(basePath: string) {
...
interface TourFiltersHeaderBarProps {
  isOpen: boolean;
...
function TourFiltersHeaderBar({
  isOpen,
  hasActiveFilters,
  currentSort,
  onToggleOpen,
  onSortChange,
}: TourFiltersHeaderBarProps) {
  return (
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            aria-label="Leitung"
            value={currentGuide}
            onChange={(e) => onUpdateFilters({ guide: e.target.value })}
            className="w-full rounded-xl border border-slate-100 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-jdav-green"
          >
...
export function TourFilters({
  ...
  const filters = useTourFilters(basePath);

  return (
    <div className="mb-6 space-y-4">
      <TourFiltersHeaderBar
        isOpen={isOpen}
        hasActiveFilters={filters.hasActiveFilters}
        currentSort={filters.currentSort}
        onToggleOpen={() => setIsOpen(!isOpen)}
        onSortChange={(sort) => filters.updateFilters({ sort })}
      />
      ...
      {isOpen && (
        <TourFiltersPanel
          ...
          currentDifficulty={filters.currentDifficulty}
          currentGuide={filters.currentGuide}
          currentGroup={filters.currentGroup}
          currentAvailability={filters.currentAvailability}
          hasActiveFilters={filters.hasActiveFilters}
          onUpdateFilters={filters.updateFilters}
