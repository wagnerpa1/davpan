    ...
    <>
      ...
      <div 
        className={cn(
          ...
          isMoreOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-full pointer-events-none"
        )}
      >
        <div className="flex h-20 items-center justify-between border-b border-slate-100 px-6 pt-safe">
          <div>
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">Menü</h2>
            <p className="text-xs text-slate-500 font-medium lowercase tracking-wider">JDAV Pfarrkirchen</p>
          </div>
          <button 
              ...
              return (
                ...
                >
                  <div className={cn(
                    ...
                    item.disabled ? "bg-slate-200 text-slate-400" : "bg-jdav-green/10 text-jdav-green"
                  )}>
             ...
             <form action="/auth/signout" method="POST">
               ...
               >
                 ...
                 <div className="text-left">
                   ...
                   <p className="text-[10px] uppercase font-bold tracking-widest opacity-60">Sitzung beenden</p>
                 </div>
               </button>
             </form>
          </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex h-20 w-full items-center justify-around border-t border-slate-100 bg-white/90 backdrop-blur-lg pb-safe-bottom pt-1 md:hidden shadow-[0_-8px_30px_rgba(0,0,0,0.04)]">
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive ? "text-jdav-green" : "text-slate-400 hover:text-slate-600"
              )}
            >
              <div className={cn(
                ...
                <Icon className={cn("h-6 w-6 transition-transform duration-300", isActive && "scale-110")} strokeWidth={isActive ? 2.5 : 2} />
              </div>
              <span className={cn(
        ...
        <button
          ...
          className={cn(
            "group relative flex flex-col items-center justify-center px-1.5 py-1 transition-all duration-300",
            isMoreOpen ? "text-jdav-green" : "text-slate-400 hover:text-slate-600"
          )}
        >
          <div className={cn(
            "mb-1 flex h-9 w-14 items-center justify-center rounded-full transition-all duration-300",
            ...
            {isMoreOpen ? (
              <X className="h-6 w-6 scale-110 transition-transform" strokeWidth={2.5} />
            ) : (
              <Menu className="h-6 w-6 transition-transform group-hover:rotate-12" strokeWidth={2} />
            )}
          </div>
          <span className={cn(
            "text-[10px] font-bold tracking-tight transition-all duration-300",
