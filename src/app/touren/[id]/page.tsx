...
import { MapPin, Calendar, Mountain, Info, Clock, Route, Ruler, Euro, Edit, Trash2, Users, Baby, ShieldCheck, Tag, Users2 } from "lucide-react";
...
export default async function TourDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isFull = (tour.max_participants || 0) > 0 && confirmedParticipants.length >= (tour.max_participants || 0);
        ...
        <div className="p-6 sm:p-10 text-slate-700">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 mb-8">
            ...
            <div className="flex flex-col items-center justify-center rounded-2xl bg-slate-50 p-4 text-center">
              ...
              <span className="mt-1 font-medium text-sm capitalize">{tour.category || 'n.A.'}</span>
            </div>
            <div className="flex flex-col items-center justify-center rounded-2xl bg-slate-50 p-4 text-center">
              ...
              <span className="mt-1 font-medium text-sm">{tour.difficulty || 'Keine'}</span>
            </div>
            <div className={cn("flex flex-col items-center justify-center rounded-2xl p-4 text-center", isFull ? 'bg-red-50' : 'bg-slate-50')}>
              <Users className={cn("mb-2 h-6 w-6", isFull ? 'text-red-500' : 'text-jdav-green')} />
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Plätze</span>
              <span className={cn('mt-1 font-medium text-sm', isFull && 'text-red-600 font-black')}>
            ...
            <div className="flex flex-col items-center justify-center rounded-2xl bg-slate-50 p-4 text-center">
              ...
              <span className="mt-1 font-medium text-sm">{tour.distance ? `${tour.distance} km` : '–'}</span>
            </div>
            <div className="flex flex-col items-center justify-center rounded-2xl bg-slate-50 p-4 text-center">
              ...
              <span className="mt-1 font-medium text-sm">{tour.duration_hours ? `${tour.duration_hours} h` : '–'}</span>
            </div>
            {tour.cost_info && (
              <div className="flex flex-col items-center justify-center rounded-2xl bg-slate-50 p-4 text-center">
                <Euro className="mb-2 h-6 w-6 text-jdav-green" />
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Kosten</span>
                <span className="mt-1 font-medium text-sm line-clamp-2">{tour.cost_info}</span>
              </div>
            )}
            {tour.min_age && (
              <div className="flex flex-col items-center justify-center rounded-2xl bg-slate-50 p-4 text-center">
                <Baby className="mb-2 h-6 w-6 text-jdav-green" />
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Mindestalter</span>
                <span className="mt-1 font-medium text-sm">{tour.min_age} Jahre</span>
              </div>
            )}
          </div>
                ...
                {cancelledParticipants.length > 0 && (
                  <details className="group rounded-2xl border border-slate-100 bg-slate-50/50">
                    <summary className="flex cursor-pointer list-none items-center justify-between p-4 font-bold text-slate-500 hover:text-slate-700 transition-colors">
                      <span className="text-xs uppercase tracking-widest">Abgelehnte / Stornierte ({cancelledParticipants.length})</span>
