...
function SystemNotificationForm({ tourGroups }: { tourGroups: TourGroup[] }) {
  return (
    <section className="flex flex-col gap-3 rounded-card border border-slate-200 bg-white p-6 shadow-sm">
      <div>
...
function NewsArchiveSection({
  newsByYear,
}: {
  newsByYear: ArchiveYearGroup<NewsPost>[];
}) {
  return (
    <section className="flex flex-col gap-3 rounded-card border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between gap-4">
        ...
        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
          {newsByYear.reduce(
            (sum, yearGroup) => sum + yearGroup.items.length,
            0,
          )}{" "}
          Einträge
        </span>
...
function AuditArchiveSection({
  auditsByYear,
  senderMap,
  groupMap,
}: {
  auditsByYear: ArchiveYearGroup<SystemAuditEntry>[];
  ...
  return (
    <section className="flex flex-col gap-3 rounded-card border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold">Systemnachrichten-Archiv</h2>
          <p className="text-sm text-slate-500">
            Versandprotokoll ebenfalls nach Jahren gruppiert und zunächst zu.
          </p>
        </div>
        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
          {auditsByYear.reduce(
...
export default async function AdminNewsPage() {
  ...
  const senderIds = audits.reduce<string[]>((ids, entry) => {
    if (entry.sent_by && !ids.includes(entry.sent_by)) {
      ids.push(entry.sent_by);
    }
    return ids;
  }, []);
  ...
  const groupMap = new Map(
    (tourGroups ?? []).map((group) => [group.id, group.group_name]),
  );
    ...
    <div className="mx-auto max-w-site px-4 py-8">
      ...
      <div className="space-y-6">
        ...
        <SystemNotificationForm
          tourGroups={(tourGroups ?? []) as TourGroup[]}
        />
        <NewsArchiveSection newsByYear={newsByYear} />
        <AuditArchiveSection
          auditsByYear={auditsByYear}
          senderMap={senderMap}
          groupMap={groupMap}
        />
      </div>
